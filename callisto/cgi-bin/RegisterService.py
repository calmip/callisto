...
class RegisterService(object):
    ...
    def update_ontology(self):
        ...
        myont = Namespace("http://callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#")
        ...
        servi = self.conn.createURI("<http://callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#"+ svc+">")
        profile = self.conn.createURI("<http://callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#" + profile_id+">")
        data_tim = self.conn.createURI("<http://callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#" + timer+">")
        data_url = self.conn.createURI("<http://callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#" + short_url+">")
        ...
        print("service name:"+"<http://callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#" + timer+">")
        general =  sys.argv[3].split(",")
        print("general: "+str(general))
        description = general[0]
        timer = str(time.time())
        data_url = self.conn.createURI("<http://callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#" +timer +">")
        soft = self.conn.createURI("<http://callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#" + general[2]+">")
        ...
        for genin in general_input:
            ...
            agg = self.conn.createURI("<http://callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#" + aggregate+">")
            self.conn.add(agg, RDF.TYPE, URIRef(arcas.Aggregate))
            self.conn.add(servi, hasInput, agg)
            try:
                quantity = details[0]
                q = self.conn.createURI("<http://callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#" + quantity +">")
                self.conn.add(agg,isCombinedToParam, q)
                form = details[1]
                f = self.conn.createURI("<http://callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#" + form +">")
                self.conn.add(agg, isCombinedToFormat, f)
                unit = details[2]
                u = self.conn.createURI("<http://callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#"+unit+">")
