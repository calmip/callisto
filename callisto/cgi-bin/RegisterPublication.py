...
class RegisterPaper(object):
    ...
    def update_ontology(self):
        ...
        myont = Namespace("http://www.callisto-dev.calmip.univ-toulouse.fr/demonstration.rdf#")
        ...
        cites = URIRef(mp.citation)

        current_repo = self.repository.upper()

        attribution = self.conn.createURI("<http://www.callisto-dev.calmip.univ-toulouse.fr/"+current_repo+".rdf#A_P" + timer +">")
        ref = self.conn.createURI("<http://www.callisto-dev.calmip.univ-toulouse.fr/"+current_repo+".rdf#ref" + timer +">")
        ...
        #self.conn.add(ref, "<http://purl.org/mp/citation>", self.ref)
        biblio = self.ref
        self.conn.add(ref,"<http://purl.org/mp/citation>" , Literal(biblio))
        url = self.url
        ...
        for claim in claims:
            #print("claim: " + claim)
            timer = str(time.time())
            cl = self.conn.createURI("<http://www.callisto-dev.calmip.univ-toulouse.fr/"+current_repo+".rdf#claim" + timer +">")
            self.conn.add(cl,  "<http://purl.org/mp/statement>", claim)
            self.conn.add(cl,  "<http://purl.org/mp/supports>", ref)
            for qualifier in qualifiers:
                #qualifier_cal = self.conn.createURI("<http://www.callisto.calmip.univ-toulouse.fr/"+current_repo+".rdf#qual" + timer +">")
                real_qual = self.conn.createURI("<http://www.callisto-dev.calmip.univ-toulouse.fr/"+current_repo+".rdf#"+qualifier+">")
                self.conn.add(cl, "<http://purl.org/mp/qualifiedBy>", real_qual)
                #self.conn.add("<http://www.callisto.calmip.univ-toulouse.fr/"+current_repo+".rdf#"+qualifier+">","<http://purl.org/mp/qualifies>",cl)
        
        # AccessURL:
        #timer = str(time.time())
        data_url = self.conn.createURI("<http://www.callisto-dev.calmip.univ-toulouse.fr/"+current_repo+".rdf#url" + timer +">")
        self.conn.add(data_url, RDF.TYPE, URIRef(arcas.AccessUrl))
        self.conn.add(data_url,"<http://purl.org/mp/elementOf>" , ref)
        ...
        log.info('Repository contains %d statement(s).' % self.conn.size())
    ...
    def open_connection(self):
        ...
        repository = catalog.getRepository(self.repository.lower(), mode)
    ...
    def __init__(self):
        ...
        """
        try:
            os.system("rm register_publication.log")
        except:
            pass
        log.basicConfig(filename='register_publication.log', level=log.DEBUG, format='%(levelname)s:%(asctime)s %(message)s ')
        self.readconf = ReadConfig.ReadConfig()
        self.read_config()
        self.form = cgi.FieldStorage()
        self.repository = str(self.form.getvalue("repo"))
        ...
        self.user = str(self.form.getvalue("user"))
        log.debug("user="+str(self.user)+".")
        self.password = str(self.form.getvalue("password"))
