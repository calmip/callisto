...
class RegisterPaper(object):
    ...
    def update_ontology(self):
        ...
        attribution = self.conn.createURI("<http://callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#A_P" + timer +">")
        ref = self.conn.createURI("<http://callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#ref" + timer +">")
        ...
        for claim in claims:
            #print("claim: " + claim)
            timer = str(time.time())
            cl = self.conn.createURI("<http://callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#claim" + timer +">")
            ...
            for qualifier in qualifiers:
                ...
                self.conn.add("callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#"+qualifier, "<http://purl.org/mp/qualifiedBy>", cl)
                #self.conn.add("<http://www.callisto.calmip.univ-toulouse.fr/SMS.rdf#"+qualifier+">","<http://purl.org/mp/qualifies>",cl)
        
        # AccessURL:
        timer = str(time.time())
        data_url = self.conn.createURI("<http://callisto-dev.calmip.univ-toulouse.fr/SMS.rdf#url" + timer +">")
